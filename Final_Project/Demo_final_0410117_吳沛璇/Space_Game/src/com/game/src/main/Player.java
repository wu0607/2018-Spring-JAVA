...
public class Player {
	...
	private BufferedImage player_normal;
	private BufferedImage player_med;
	private BufferedImage player_big;
	private BufferedImage player_thin;
	...
	public Player(double x, double y, Game game) {
		...
		player_normal = ss.grabImage(0, 0, 64, 64);
		player_med = ss.grabImage(100, 300, 63, 63);
		player_big = ss.grabImage(164, 300, 63, 63);
		player_thin = ss.grabImage(228, 300, 63, 63);
...
//////////Win
	...
	public void render(Graphics g) {
		if(curr_weight < 40)g.drawImage(player_thin, (int)x, (int)y, null);
		else if(curr_weight < 60)g.drawImage(player_normal, (int)x, (int)y, null);
		else if(curr_weight < 80)g.drawImage(player_med, (int)x, (int)y, null);
		else g.drawImage(player_big, (int)x, (int)y, null);
	...
	public void subweight(double quantity) {
		if(curr_weight>0.1 && curr_weight>ideal_weight) {
			this.curr_weight -= quantity;
		}
		if(curr_weight < 0) {
			this.curr_weight = 0.1;
		}
	}
	public void addweight(double quantity) {
		this.curr_weight += quantity;
